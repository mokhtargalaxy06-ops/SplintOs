...
int main(int argument_count, char **arguments)
{
    ...
    struct splint_poll_entry readiness[2] = {
        {pipe_descriptors[0], SPLINT_POLL_READ, 0},
        {pipe_descriptors[1], SPLINT_POLL_WRITE, 0},
    };
    if (sys_poll(readiness, 2, 0) != 1 || readiness[0].returned_events != 0 ||
        readiness[1].returned_events != SPLINT_POLL_WRITE) return 2;
    ...
    readiness[0].returned_events = 0;
    if (sys_poll(readiness, 1, 200) != 1 ||
        readiness[0].returned_events != SPLINT_POLL_READ) return 6;
    char buffer[64];
