...
struct udp_socket {
    bool used, pending;
    uint16_t port, source_port, length;
    uint8_t source_ip[4];
                        ...
                        bool broadcast)
{
    ...
    const uint8_t *destination_mac = broadcast ? NULL : arp_lookup(destination_ip);
    if (!broadcast && destination_mac == NULL) {
        arp_request(destination_ip);
        return -2;
    }
...
static void handle_dhcp(const uint8_t *payload, size_t length)
{
    ...
    if (packet->operation != 2 || packet->transaction_id != dhcp_transaction ||
        !memory_equal(packet->client_hardware, mac, 6)) return;
    uint8_t server[4];
    uint8_t type = dhcp_option(packet, length, 53, NULL);
    if (type == 2 && dhcp_option(packet, length, 54, server) != 0) {
...
static void handle_arp(uint8_t *frame, uint16_t length)
{
    ...
    struct ethernet_header *ethernet = (struct ethernet_header *)frame;
    struct arp_packet *arp = (struct arp_packet *)(ethernet + 1);
    if (arp->hardware_size != 6 || arp->protocol_size != 4) return;
                       ...
                       uint8_t header_length)
{
    ...
    for (uint8_t i = 0; i < UDP_SOCKET_COUNT; ++i) {
        ...
        memory_copy(udp_sockets[i].payload, payload, payload_length);
        memory_copy(udp_sockets[i].source_ip, ipv4->source, 4);
        udp_sockets[i].source_port = swap16(udp->source_port);
        udp_sockets[i].length = (uint16_t)payload_length;
        udp_sockets[i].pending = true;
        return;
    }
}
                ...
                uint16_t *source_port)
{
    if (socket < 0 || socket >= UDP_SOCKET_COUNT || !udp_sockets[socket].used ||
        !udp_sockets[socket].pending || data == NULL) return -1;
    struct udp_socket *entry = &udp_sockets[socket];
    size_t length = entry->length < capacity ? entry->length : capacity;
    memory_copy(data, entry->payload, length);
    if (source_ip != NULL) memory_copy(source_ip, entry->source_ip, 4);
    if (source_port != NULL) *source_port = entry->source_port;
    entry->pending = false;
    return (int)length;
}
